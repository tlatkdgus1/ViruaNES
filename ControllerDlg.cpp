...
// コ?ンド
DLG_COMMAND_BEGIN()
DLG_ON_COMMAND_NOTIFY_RANGE( IDC_CTR_UP, IDC_CTR_EXB, STN_CLICKED, OnClicked )
DLG_COMMAND_END()
DLG_MESSAGE_END()
...
void	CControllerDlg::OnPageSetup( UINT nPage )
{
	...
	INT	nKey;
	LPCSTR	lpStr;
	INT	nOffset = m_SettingSel?10:0;

	for( INT nID = IDC_CTR_UP; nID <= IDC_CTR_EXB; nID++ ) {
