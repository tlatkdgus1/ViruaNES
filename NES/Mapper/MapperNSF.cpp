...
BYTE	MapperNSF::NsfPlayerCode[] = {
	0x78, 0xD8, 0xA2, 0xFF, 0x9A, 0xAD, 0x02, 0x20, 0x10, 0xFB, 0xA9, 0x80, 0x8D, 0x00, 0x20, 0xA9,
	0x00, 0x8D, 0x01, 0x20, 0xA9, 0x00, 0x8D, 0xF2, 0x47, 0xAD, 0xF1, 0x47, 0xF0, 0xFB, 0xA9, 0x00,
	0x8D, 0xF1, 0x47, 0xAD, 0xF0, 0x47, 0x20, 0x00, 0x80, 0xA9, 0x01, 0x8D, 0xF2, 0x47, 0xA9, 0x00,
	0xF0, 0xE7, 0x48, 0x8A, 0x48, 0x98, 0x48, 0xAD, 0xF2, 0x47, 0xF0, 0x03, 0x20, 0x03, 0x80, 0x68,
	0xA8, 0x68, 0xAA, 0x68, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xD2, 0x47, 0xA0, 0x47, 0xD2, 0x47,
};
...
INT	i;
	...
	// Load Player Routine($47A0-$47FF)
	for( i = 0; i < 0x0060; i++ ) {
		WRAM[0xA7A0+i] = NsfPlayerCode[i];
	}
	// Song init & play address
	WRAM[0xA7C7] = (BYTE) nsfheader.InitAddress;
	WRAM[0xA7C8] = (BYTE)(nsfheader.InitAddress>>8);
	WRAM[0xA7DD] = (BYTE) nsfheader.PlayAddress;
	WRAM[0xA7DE] = (BYTE)(nsfheader.PlayAddress>>8);

	// For Play routine vector
	WRAM[0x9FFA] = NsfPlayerCode[0x5A];
	WRAM[0x9FFB] = NsfPlayerCode[0x5B];
	WRAM[0x9FFC] = NsfPlayerCode[0x5C];
	WRAM[0x9FFD] = NsfPlayerCode[0x5D];
	WRAM[0x9FFE] = NsfPlayerCode[0x5E];
	WRAM[0x9FFF] = NsfPlayerCode[0x5F];
...
void	MapperNSF::VSync()
{
	...
	if( padpush & (1<<0) ) {
		...
		WRAM[0xA7F0] = songno;	// SONG_NO
		WRAM[0xA7F1] = 1;	// SONGSTART
		WRAM[0xA7F2] = 0;	// INT_ENABLE
	}
