...
APU_N106::APU_N106()
{
	...
	// âºê›íË
	cycle_rate = (DWORD)(APU_CLOCK*12.0f*(1<<20)/(45.0f*22050.0f));
}
...
void	APU_N106::Setup( INT nRate )
{
	cycle_rate = (DWORD)(APU_CLOCK*12.0f*(1<<20)/(45.0f*nRate));
}
...
INT	APU_N106::GetFreq( INT channel )
{
	if( channel < 8 ) {
		...
		if( !temp )
			return	0;
		return	(INT)(256.0*(double)APU_CLOCK*12.0*ch->freq/((double)0x40000*temp));
	}
